...
export const MOCK_STRATEGIES: Strategy[] = [
  ...
  {
    id: "st-limit-down-watch",
    name: "A股 ST 连续跌停观察策略",
    description: "筛选连续跌停5-50板、尚未见底的ST股票进入观察池。⚠️ 纯观察策略，不产生买入信号，所有入池标的显示「观察中，暂不买入」。连续跌停股票可能退市或无法卖出，极高风险，仅供研究。",
    marketCondition: "全市场",
    annualReturn: 0,
    maxDrawdown:  0,
    winRate:   0,
    tradeCount: 0,
    riskLevel: "高",
    indicators: ["连续跌停检测", "见底检测", "退市风险评分", "重组预期评分", "财务修复评分"],
    markets: ["A"],
    params: [
      { key: "minLimitDownStreak",    label: "最少连续跌停",   defaultValue: 5,  min: 3,  max: 20, step: 1, unit: "板" },
      { key: "maxLimitDownStreak",    label: "最多连续跌停",   defaultValue: 50, min: 10, max: 100, step: 5, unit: "板" },
      { key: "maxDelistingRiskScore", label: "最大退市风险分", defaultValue: 40, min: 20, max: 80, step: 5, unit: "分" },
    ],
    tags: ["ST专题", "连续跌停", "观察池", "纯观察", "不买入", "极高风险"],
  },
  {
    id: "st-limit-down-first-limit-up",
    name: "A股 ST 连续跌停后首板策略",
    description: "扫描连续跌停5-50板之后出现第一个涨停板的ST股票，排除明显退市风险，结合重组/修复预期和首板强度，作为高风险买入候选。⚠️ 极高风险：首板涨停可能是假反弹，仅供研究和模拟，不构成投资建议。",
    marketCondition: "全市场",
    annualReturn: 0,
    maxDrawdown:  0,
    winRate:   0,
    tradeCount: 0,
    riskLevel: "高",
    indicators: ["连续跌停检测", "首板涨停识别", "退市风险评分", "重组预期评分", "首板强度评分"],
    markets: ["A"],
    params: [
      { key: "minLimitDownStreak",           label: "最少连续跌停",     defaultValue: 5,  min: 3,  max: 20, step: 1, unit: "板" },
      { key: "maxLimitDownStreak",           label: "最多连续跌停",     defaultValue: 50, min: 10, max: 100, step: 5, unit: "板" },
      { key: "maxDelistingRiskScore",        label: "最大退市风险分",   defaultValue: 40, min: 20, max: 80, step: 5, unit: "分" },
      { key: "minFirstLimitUpStrengthScore", label: "最低首板强度分",   defaultValue: 60, min: 40, max: 90, step: 5, unit: "分" },
      { key: "stopLossPercent",              label: "止损比例",         defaultValue: 5,  min: 3,  max: 15, step: 1, unit: "%" },
      { key: "takeProfitPercent",            label: "止盈比例",         defaultValue: 15, min: 5,  max: 50, step: 5, unit: "%" },
    ],
    tags: ["ST专题", "连续跌停", "首板", "高风险", "重组预期", "模拟盘", "回测"],
  },
  {
