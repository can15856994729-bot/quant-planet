 ...
 *
 * 获取上市公司现金流量表（合并报表），按报告期降序返回。
 * 数据来源：Tushare cashflow 接口
 * 缓存：7d（服务端内存缓存） + HTTP Cache-Control 24h
 */
import { NextRequest, NextResponse } from "next/server";
import { getCashflow, hasTushareToken, daysAgoStr, todayStr } from "@/lib/tushareService";
...
export async function GET(req: NextRequest) {
  ...
  const res = await getCashflow(tsCode, startDate, endDate);
  if (!res.ok) {
    return NextResponse.json({
      ok: false,
      error: res.error,
