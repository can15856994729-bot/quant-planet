/**
 * GET /api/tushare/balancesheet?tsCode=xxx&startDate=YYYYMMDD&endDate=YYYYMMDD
 *
 * 获取上市公司资产负债表（合并报表），按报告期降序返回。
 * 数据来源：Tushare balancesheet 接口
 * 缓存：7d（服务端内存缓存） + HTTP Cache-Control 24h
 */
import { NextRequest, NextResponse } from "next/server";
import { getBalanceSheet, hasTushareToken, daysAgoStr, todayStr } from "@/lib/tushareService";
...
export async function GET(req: NextRequest) {
  ...
  const res = await getBalanceSheet(tsCode, startDate, endDate);
  if (!res.ok) {
    return NextResponse.json({
      ok: false,
      error: res.error,
