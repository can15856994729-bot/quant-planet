/**
 * GET /api/strategies/bom-supply-chain-stock-strategy/chokepoint-pools/cpo
 *
 * CPO 咽喉池 API
 * 流程：
 *   1. 从 cpoChokepointPoolService 获取全部内置 CPO 咽喉池股票
...
import {
  CPO_CHOKEPOINT_STOCKS,
  CPO_CHOKEPOINT_CORE_POOL,
  CPO_CHOKEPOINT_GROUPS,
  CPO_CHOKEPOINT_RISKS,
  calcCpoChokepointScore,
} from "@/lib/cpoChokepointPoolService";
...
export async function GET(): Promise<NextResponse> {
  ...
  if (hasTushareToken()) {
    ...
    const allTsCodes = CPO_CHOKEPOINT_STOCKS.map(s => s.tsCode).join(",");
    ...
    if (res?.ok && res.records.length > 0) {
      ...
      const fb = await callTushare(
        "daily_basic",
        { ts_code: CPO_CHOKEPOINT_CORE_POOL.map(s => s.tsCode).join(","), trade_date: "" },
        "ts_code,pe_ttm,pb,total_mv,turnover_rate,close",
        30 * 60 * 1000,
      ).catch(() => null);
      if (fb?.ok) {
  ...
  const stocks = CPO_CHOKEPOINT_STOCKS.map(s => {
    const fin   = finMap.get(s.tsCode);
    const score = calcCpoChokepointScore(s, { pe: fin?.pe ?? null, pb: fin?.pb ?? null });
    return {
      tsCode:              s.tsCode,
      symbol:              s.symbol,
      name:                s.name,
  ...
  const groups = CPO_CHOKEPOINT_GROUPS.map(g => ({
    ...g,
    stocks: stocks
