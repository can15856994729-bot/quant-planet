/**
 * app/api/strategies/trend-correction-mini-reversal/backtest/route.ts
 *
 * POST /api/strategies/trend-correction-mini-reversal/backtest
 * A股趋势回调微型反转策略 — 单只股票历史回测
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await backtestTrendCorrectionStock(tsCode, name, params);
    if (!result) {
      return NextResponse.json({ ok: false, error: "未获取到 K线数据或数据不足" }, { status: 404 });
    }
