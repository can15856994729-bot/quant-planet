...
/**
 * PCBChokepointPool.tsx — PCB 咽喉池
 *
 * PCB 产业链中真正卡脖子的核心环节：高速覆铜板/CCL、高多层PCB、
 * 光模块高速互联PCB、高端设备耗材。
 *
...
import {
  ...
  ChevronDown, ChevronRight, Cpu, Zap, Layers,
} from "lucide-react";
import { PCB_CHOKEPOINT_GROUPS, type PcbChokepointGroupKey } from "@/lib/pcbChokepointPoolService";
...
function groupColor(key: string) {
  return PCB_CHOKEPOINT_GROUPS.find(g => g.key === key)?.color ?? "#64748b";
}
...
interface StockData {
  ...
  score: {
    premiumMaterialScore: number; highSpeedAppScore: number;
    multilayerMfgScore: number; aiServerBenefitScore: number;
...
function StockCard({ stock, refresh }: { stock: StockData; refresh: () => void }) {
  ...
  function handleWatch(e: React.MouseEvent) {
    ...
    if (inWatch) {
      ...
      addToWatchlist({
        ...
        industry: "PCB / 印制电路板", currency: "CNY",
      });
  ...
  function handleBom(e: React.MouseEvent) {
    ...
    if (inBom) {
      ...
      addBomCand({ symbol: stock.symbol, name: stock.name, tsCode: stock.tsCode, sourcePool: "PCB 咽喉池" });
            ...
            <div className="flex flex-wrap items-center gap-1">
              {stock.groups.slice(0, 2).map(gk => (
                <span key={gk} className="text-[9px] px-1.5 py-0.5 rounded-full font-semibold"
                  ...
                  {PCB_CHOKEPOINT_GROUPS.find(g => g.key === gk)?.groupName ?? gk}
                </span>
              ))}
            </div>
          </div>
      ...
      {expanded && (
        <div className="px-3 pb-3" style={{ borderTop: "1px solid #1a2f50" }}>
          <div className="pt-2 space-y-3">
            {/* 咽喉点描述 */}
            <div className="p-2 rounded-xl" style={{ background: "rgba(0,229,168,0.05)", border: "1px solid rgba(0,229,168,0.12)" }}>
              <p className="text-[10px] font-semibold mb-0.5" style={{ color: "#00E5A8" }}>⚡ 咽喉点</p>
            ...
            {/* 评分六维 */}
            <div>
              <p className="text-[10px] mb-1.5" style={{ color: "#475569" }}>咽喉点评分（PCB维度）</p>
              <div className="grid grid-cols-3 gap-1.5">
                {[
                  { label: "高端材料", score: sc.premiumMaterialScore,  w: "25%" },
                  { label: "高速高频", score: sc.highSpeedAppScore,      w: "20%" },
                  { label: "高多层制造",score: sc.multilayerMfgScore,    w: "20%" },
                  { label: "AI服务器",  score: sc.aiServerBenefitScore,  w: "15%" },
...
interface ApiResponse {
  ...
  corePool: StockData[];
  groups: Array<{
    key: PcbChokepointGroupKey; groupName: string; chokePoint: string;
...
type ActiveTab = "core" | PcbChokepointGroupKey;

// ── 主组件 ────────────────────────────────────────────────────────
export default function PCBChokepointPool() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res  = await fetch("/api/strategies/bom-supply-chain-stock-strategy/chokepoint-pools/pcb", { cache: "no-store" });
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-16 gap-3">
      ...
      <p className="text-[12px]" style={{ color: "#64748b" }}>加载 PCB 咽喉池...</p>
    </div>
  );
    ...
    <div>
      ...
      <div className="p-3 rounded-2xl mb-3" style={{ background: "#0d1f3c", border: "1px solid #1a2f50" }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[13px] font-bold" style={{ color: "#e2e8f0" }}>PCB 咽喉池</span>
          {data.hasFinancial && (
        ...
        <p className="text-[10px] mb-2 leading-relaxed" style={{ color: "#64748b" }}>
          PCB 产业链真正的卡脖子环节：高速覆铜板/CCL材料、高多层PCB制造、
          800G光模块配套PCB、高端钻机/激光/电镀设备。
        </p>
      ...
      {activeTab === "core" ? (
        <div className="p-3 rounded-2xl mb-3"
          ...
          <p className="text-[10px] font-semibold mb-1" style={{ color: "#00E5A8" }}>⭐ PCB 咽喉核心重点池（{data.coreCount} 只）</p>
          <p className="text-[10px] leading-relaxed" style={{ color: "#64748b" }}>
            精选 PCB 产业链各咽喉环节代表性标的：CCL材料龙头、高多层PCB龙头、高端设备龙头。
            点击展开查看咽喉点评分和操作按钮。
          </p>
        </div>
      ) : activeGroup && (
      ...
      <div className="mt-4 p-3 rounded-2xl" style={{ background: "#0d1f3c", border: "1px solid #1a2f50" }}>
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-[11px] font-bold" style={{ color: "#ef4444" }}>PCB 咽喉池风险提示</span>
        </div>
      ...
      <div className="mt-3 p-3 rounded-2xl" style={{ background: "#0d1f3c", border: "1px solid #1a2f50" }}>
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-[10px] font-bold" style={{ color: "#94a3b8" }}>PCB 咽喉关键词</span>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {["高速CCL", "低Dk/Df", "高多层PCB", "HDI背板", "AI服务器PCB", "800G光模块PCB",
            "LDI激光直接成像", "PCB微型钻头", "电镀设备", "球形硅微粉", "国产替代", "高频高速材料"].map(t => (
            <span key={t} className="text-[9px] px-1.5 py-0.5 rounded-full"
              style={{ background: "rgba(0,229,168,0.08)", color: "#94a3b8", border: "1px solid rgba(0,229,168,0.15)" }}>
              {t}
            </span>
          ))}
        </div>
